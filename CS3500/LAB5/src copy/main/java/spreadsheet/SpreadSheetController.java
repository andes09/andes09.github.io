...
public class SpreadSheetController {
  ...
  public void control() throws IllegalStateException {
    ...
    while (!quit && scan.hasNext()) { //continue until the user quits
      writeMessage("Type instruction: "); //prompt for the instruction name
      String userInstruction = scan.next(); //take an instruction name
      if (userInstruction.equals("quit") || userInstruction.equals("q")) {
        quit = true;
      } else {
        processCommand(userInstruction, scan, sheet);
      }
    }
  ...
  /**
   * Processes the given instruction and executes it on the given spreadsheet.
   *
   * @param userInstruction the instruction to execute
   * @param scan the scanner to read from
   * @param sheet the spreadsheet to modify or read from
   */
  protected void processCommand(String userInstruction, Scanner scan, SpreadSheet sheet) {
    int row;
    int col;
    double value;
    ...
    switch (userInstruction) {
      case "assign-value": //assign a value to a cell
        try {
          row = getRowNum(scan.next()); //get in the row string
          col = scan.nextInt(); //get in the column number, starting with 1
      ...
      case "print-value": //print a value from the cell
        try {
          row = getRowNum(scan.next()); //get the row string
          col = scan.nextInt(); //get the column number, starting with 1
   ...
   *
   * @param rowLetters the letter to convert
   * @throws IllegalArgumentException if the argument has a non-letter character
   */
  ...
  /**
   * Prints the welcome message and menu for the user.
   *
   * @throws IllegalStateException if I/O fails for any reason
   */
